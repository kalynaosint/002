...
const LOSS_COLORS = [
  "#4a4632", "#f59a00", "#777777", "#b35c00", "#8d6e63", "#333333",
  "#c9a227", "#6d5d3b", "#9e9e9e", "#d98200", "#5f5a3f", "#2f2b20",
  "#bdbdbd", "#ffb74d"
];
...
function renderCombatDirectionList() {
  ...
  overviewBtn.className = "direction-btn" + (selectedCombatDirection === null ? " active" : "");
  overviewBtn.textContent = "全部方向总览";
  overviewBtn.addEventListener("click", () => {
    selectedCombatDirection = null;
    renderCombatTab();
  });
  box.appendChild(overviewBtn);
...
function renderLossCategoryList() {
  ...
  const overviewBtn = document.createElement("button");
  overviewBtn.type = "button";
  overviewBtn.className = "direction-btn" + (selectedLossCategory === null ? " active" : "");
  overviewBtn.textContent = "损失占比总览";
  overviewBtn.addEventListener("click", () => {
      ...
      lossChart,
      {
        ...
        data: {
          ...
          datasets: [{
            ...
            backgroundColor: LOSS_CATEGORIES.map((_, i) => LOSS_COLORS[i % LOSS_COLORS.length]),
            borderWidth: 1
          }]
        },
        options: {
          ...
          maintainAspectRatio: false,
          plugins: {
            title: { display: true, text: "俄军损失占比总览" },
