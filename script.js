...
function toggleChangesOverlay() {
  if (!selectedDate || !selectedDirection) return;
...
function combatDirectionsForMainChart() {
  return state.directions.filter(direction => direction !== UNKNOWN_DIRECTION);
}

function attackCountValues(labels) {
  return labels.map(date => numberValue(state.days[date]?.combat?.[UNKNOWN_DIRECTION]));
}
...
function combatOverviewValues(labels) {
  return labels.map(date => {
    return combatDirectionsForMainChart().reduce((sum, direction) => {
...
function buildCombatOverviewDatasets(labels) {
  const datasets = combatDirectionsForMainChart().map(direction => {
...
function renderCombatDirectionList() {
  ...
  combatDirectionsForMainChart().forEach(direction => {
    const btn = document.createElement("button");
    btn.type = "button";
...
function renderAttackCountChart(labels, isOverview) {
  const wrap = document.getElementById("attackCountOverviewWrap");
  const canvas = document.getElementById("attackCountChart");

  if (!wrap || !canvas) return;

  wrap.classList.toggle("hidden", !isOverview);

  if (!isOverview) {
    if (attackCountChart) {
      attackCountChart.destroy();
      attackCountChart = null;
    }
    return;
  }

  const values = attackCountValues(labels);
  const datasets = [
    chartDataset("进攻次数", values, "bar", {
...
function renderCombatTab() {
  ...
  renderAttackCountChart(labels, isOverview);
