...
let selectedCombatDirection = DEFAULT_DIRECTIONS[0];
let selectedLossCategory = LOSS_CATEGORIES[0];
let combatRange = 60;
let lossRange = 60;
...
function sampleState() {
  ...
  [yesterday, today].forEach((date, idx) => {
    base.days[date] = {
      report: idx === 1
        ? "这里是当天文字报告示例。点击报告中的方向名称，例如 波克罗夫斯克、哈尔科夫、赫尔松，可以跳转到对应方向图片集。"
        : "昨日文字报告示例。哈尔科夫 与 库普扬斯克 方向继续交战。",
      images: Object.fromEntries(base.directions.map(d => [d, ""])),
      combat: Object.fromEntries(base.directions.map((d, i) => [d, Math.max(0, 3 + i + idx * 2)])),
      losses: Object.fromEntries(LOSS_CATEGORIES.map((c, i) => [c, i === 0 ? 900 + idx * 60 : Math.max(0, Math.round((i + 1) * 1.7 + idx))]))
    };
  });
...
async function loadState() {
  if (isAdmin) {
    ...
    if (raw) {
      try {
        ...
        console.warn("Local admin JSON invalid; using sample.", err);
      }
    }
    state = sampleState();
    saveLocalState();
  ...
  try {
    ...
    console.warn("Using local/sample data because data/data.json failed:", err);
    const raw = localStorage.getItem(LOCAL_STORAGE_KEY);
    if (raw) {
      try {
        state = normalizeState(JSON.parse(raw));
...
function chartDataset(label, data, type = "bar") {
  if (type === "line") {
    return {
      type: "line",
      label,
...
function addMovingAverageDatasets(datasets, values, prefix, ids) {
  ids.forEach(id => {
    const checkbox = document.getElementById(prefix + "MA" + id);
    if (checkbox?.checked) {
      datasets.push(chartDataset(`${id}日均线`, movingAverage(values, id), "line"));
    }
  });
}

function makeChart(ctx, oldChart, title, labels, datasets) {
...
function renderCombatDirectionList() {
  ...
  state.directions.forEach(direction => {
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = "direction-btn" + (direction === selectedCombatDirection ? " active" : "");
    btn.textContent = direction;
...
function renderCombatTab() {
  ...
  const values = labels.map(date => numberValue(state.days[date]?.combat?.[selectedCombatDirection]));
  const datasets = [chartDataset("击退进攻次数", values)];
  addMovingAverageDatasets(datasets, values, "combat", [3, 7, 30]);

  const title = document.getElementById("combatChartTitle");
  if (title) title.textContent = `交战数据 · ${selectedCombatDirection}`;

  combatChart = makeChart(
    document.getElementById("combatChart"),
    combatChart,
    `每日击退进攻次数 · ${selectedCombatDirection}`,
    labels,
    datasets
  );
}
...
function renderLossCategoryList() {
  ...
  LOSS_CATEGORIES.forEach(category => {
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = "direction-btn" + (category === selectedLossCategory ? " active" : "");
...
function renderLossTab() {
  ...
  const datasets = [chartDataset("每日损失", values)];
  addMovingAverageDatasets(datasets, values, "loss", [3, 7, 30]);

  const title = document.getElementById("lossChartTitle");
  if (title) title.textContent = `俄军损失 · ${selectedLossCategory}`;

  lossChart = makeChart(
    document.getElementById("lossChart"),
    lossChart,
    `每日俄军损失 · ${selectedLossCategory}`,
    labels,
    datasets
  );
}
...
function bindChartControls() {
  document.querySelectorAll("[data-combat-range]").forEach(btn => {
    btn.addEventListener("click", () => {
      combatRange = Number(btn.dataset.combatRange);
      document.querySelectorAll("[data-combat-range]").forEach(b => b.classList.remove("range-active"));
      btn.classList.add("range-active");
      renderCombatTab();
    });
  });

  document.querySelectorAll("[data-loss-range]").forEach(btn => {
    btn.addEventListener("click", () => {
      lossRange = Number(btn.dataset.lossRange);
      document.querySelectorAll("[data-loss-range]").forEach(b => b.classList.remove("range-active"));
      btn.classList.add("range-active");
      renderLossTab();
    });
  });

  ["combatMA3", "combatMA7", "combatMA30"].forEach(id => {
    document.getElementById(id)?.addEventListener("change", renderCombatTab);
  });

  ["lossMA3", "lossMA7", "lossMA30"].forEach(id => {
    document.getElementById(id)?.addEventListener("change", renderLossTab);
  });
}
...
function bindAdmin() {
  ...
  document.getElementById("saveAllBtn")?.addEventListener("click", () => {
    if (saveAdminFormToState()) alert("已保存到浏览器本地。");
  });
...
(async function init() {
  ...
  selectedDate = getLatestDate();
  if (selectedDate) calendarMonth = selectedDate.slice(0, 7);
  selectedCombatDirection = state.directions[0] || DEFAULT_DIRECTIONS[0];
